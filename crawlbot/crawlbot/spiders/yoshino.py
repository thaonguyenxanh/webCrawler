...
class YoshinoSpider(scrapy.Spider):

    start_urls = ['http://yoshino.com.vn/product-category/sp/']
    ...
    def parse(self, response):
        for elements in response.css('ul.menu li')[3:8]:
            url = elements.css('a::attr(href)').extract_first()
            name = elements.css('a span::text').extract_first()
            name = name.replace('/','')
            request = scrapy.Request(url,callback=self.parse_first_level,meta={'root_name':name})
            yield request
    
    def parse_first_level(self, response):
        product_urls = response.css("div.product-wrapper div.meta-wrapper h3 a::attr(href)").extract()
        print(product_urls)
